...
try {  
    ...
    switch (logMessageType) {  
        ...
        case "log:thread-admins": {  
            if (logMessageData.ADMIN_EVENT == "add_admin") {
                ...
                if (global.configModule[this.config.name].sendNoti) 
                    api.sendMessage(
                        `»» NOTICE ««\n${name}\nAdmin Power Activate! 🧙‍♂️🔮\nঅফিসিয়ালি এখন তুই VIP 😎🎩`,
                        threadID,
                        async (error, info) => {  
                ...
                if (global.configModule[this.config.name].sendNoti) 
                    api.sendMessage(
                        `»» NOTICE ««\n${name}\nবেশি চুল পাকনামি করার কারণে🥴\nতোকে এডমিন থেকে লাথি মেরে বের করে দেওয়া হল 😀😂`,
                        threadID,
                        async (error, info) => {  
                            if (global.configModule[this.config.name].autoUnsend) {  
                                await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));  
        ...
        case "log:user-nickname": {  
            ...
            if (global.configModule[this.config.name].sendNoti) 
                api.sendMessage(`»» NOTICE «« Update user nicknames ${logMessageData.participant_id} to: ${(logMessageData.nickname.length == 0) ? "original name": logMessageData.nickname}`, 
                threadID, async (error, info) => {  
                    if (global.configModule[this.config.name].autoUnsend) {  
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name"].timeToUnsend * 1000));  
                        return api.unsendMessage(info.messageID);  
        ...
        case "log:thread-name": {  
            ...
            if (global.configModule[this.config.name].sendNoti) 
                api.sendMessage(`»» NOTICE «« Update the group name to ${dataThread.threadName}`, 
                threadID, async (error, info) => {  
                    if (global.configModule[this.config.name].autoUnsend) {  
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name"].timeToUnsend * 1000));  
                        return api.unsendMessage(info.messageID);  
